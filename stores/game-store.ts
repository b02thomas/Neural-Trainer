...
interface GameStore extends GameState {
  ...
  loadHistoricalData: (rounds: RoundResult[], bestStreak: number) => void;
  getSessionRounds: () => RoundResult[];  // Get rounds from current session only
}

const DEFAULT_TOTAL_ROUNDS = 20;

// Track session start for filtering rounds
...
export const useGameStore = create<GameStore>()((set, get) => ({
  ...
  startGame: (totalRounds = DEFAULT_TOTAL_ROUNDS) => {
    ...
    sessionStartTime = Date.now(); // Mark session start
    ...
    set({
      ...
      rounds: [],
      currentStreak: 0,
      // Keep bestStreak from previous sessions
  ...
  resetGame: () => {
    ...
    set({
      ...
      rounds: [],
      currentStreak: 0,
      bestStreak: state.bestStreak, // Keep best streak
      totalRounds: DEFAULT_TOTAL_ROUNDS,
      currentRoundNumber: 0,
      activeColors: [...BASE_COLORS],
      buttonOrder: [...BASE_COLORS],
  ...
  loadHistoricalData: (rounds: RoundResult[], bestStreak: number) => {
    set({
      rounds,
      bestStreak,
    });
  ...
  // Get only rounds from current game session (for saving to Supabase)
  getSessionRounds: () => {
    const state = get();
    return state.rounds.filter(r => r.timestamp >= sessionStartTime);
  },
}));
